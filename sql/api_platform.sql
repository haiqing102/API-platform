...
) ENGINE = InnoDB AUTO_INCREMENT = 1798715040332218370 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '每日签到表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `payment_info`  (
  ...
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1796485194906566659 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '付款信息表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `product_order`  (
  ...
  `status` varchar(256) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT 'NOT_PAY' COMMENT '交易状态(SUCCESS：支付成功 REFUND：转入退款 NOTPAY：未支付 CLOSED：已关闭 REVOKED：已撤销（仅付款码支付会返回）\r\n                                                                              USERPAYING：用户支付中（仅付款码支付会返回）PAYERROR：支付失败（仅付款码支付会返回）)',
  ...
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1798714914750562307 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '商品订单表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `recharge_activity`  (
  ...
  `is_delete` tinyint(4) NOT NULL DEFAULT 0 COMMENT '是否删除',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1796485195288248323 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '充值活动表' ROW_FORMAT = Dynamic;
