...
CREATE TABLE `payment_info`  (
  ...
  `trade_state` varchar(256) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '交易状态(SUCCESS：支付成功 REFUND：转入退款 NOT_PAY：未支付 CLOSED：已关闭 REVOKED：已撤销（仅付款码支付会返回）\n                                                                              USERPAYING：用户支付中（仅付款码支付会返回）PAYERROR：支付失败（仅付款码支付会返回）)',
...
CREATE TABLE `product_order`  (
  ...
  `status` varchar(256) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT 'NOT_PAY' COMMENT '交易状态(SUCCESS：支付成功 REFUND：转入退款 NOT_PAY：未支付 CLOSED：已关闭 REVOKED：已撤销（仅付款码支付会返回）\r\n                                                                              USERPAYING：用户支付中（仅付款码支付会返回）PAYERROR：支付失败（仅付款码支付会返回）)',
